...
def bulk_plan():
    ...
    rows=db.execute("""SELECT v.id,v.title,v.estimated_bytes,
      ...
      SUM(CASE WHEN i.wanted=1 AND i.status NOT IN ('COMPLETE','VERIFIED','INSTALLED') THEN 1 ELSE 0 END) missing
      FROM volumes v LEFT JOIN volume_covers vc ON vc.volume_id=v.id
      LEFT JOIN items i ON i.id=vc.item_id
      WHERE v.authorized=1 AND v.status NOT IN ('COMPLETE','QUEUED','DOWNLOADING')
    ...
    return sorted(out,key=lambda x:x["coverage_score"],reverse=True)
